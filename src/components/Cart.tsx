...
const Cart = () => {
    const { cart, inc, dec, rem, totalPrice } = useProcessCartStore()
                ...
                <div className="sm:flex shadow-md my-10">
                    <div className="  w-full  sm:w-3/4 bg-white px-10 py-10">
                        <div className="flex justify-between border-b pb-8">
                            ...
                            {cart.length > 0 && <h2 className="font-semibold text-2xl">{cart.length} Items</h2>}
                        </div>
                            ...
                            return (
                                <div key={item.id.toString() + index} className="md:flex items-strech py-8 md:py-10 lg:py-8 border-t border-gray-50">
                                    ...
                                    <div key={item.id.toString() + index} className="md:pl-3 md:w-8/12 2xl:w-3/4 flex flex-col justify-center">
                                        ...
                                        <Button variant={"secondary"} onClick={() => rem(item.id)} className=" text-xs leading-3 text-red-600 mt-4">Remove from Cart</Button>
                                        <div className="flex items-center justify-between pt-5">
                                            <div className="flex items-center gap-4">
                                                <Button onClick={() => inc(item.id)} className="text-xs leading-3  cursor-pointer">Add Item</Button>
                                                ...
                                                <Button onClick={() => dec(item.id)} className="text-xs leading-3  cursor-pointer">Remove Item</Button>
                                            </div>
